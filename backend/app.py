...
# In-memory cart storage: {carrito_id: {"items": [...], "created_at": timestamp}}
CARRITOS_TEMPORALES = {}
...
def order_tracking_status():
    ...
    print(f"Order tracking request for: {email}")
...
# ---------------------------------------------------------------------------
# Routes — cart link (single-step)
# ---------------------------------------------------------------------------

@app.route('/create_cart_link', methods=['POST'])
def create_cart_link():
    """
    Build a Shopify cart URL in one step.

    Format A – string:  {"items": "variant_id:qty,variant_id:qty"}
    Format B – array:   {"items": [{"variant_id": 123, "quantity": 2}, ...]}
    """
    data = request.get_json()
...
# ---------------------------------------------------------------------------
# Routes — multi-step cart (crear / agregar / obtener link)
# ---------------------------------------------------------------------------

@app.route('/crear_carrito', methods=['POST'])
def crear_carrito():
    """Creates a temporary cart and returns a 6-digit cart ID."""
    ...
    print(f"New cart created: {carrito_id}")
...
def agregar_producto():
    """Adds a product variant to an existing temporary cart."""
    limpiar_carritos_viejos()
    ...
    print(f"Added variant {vid_int} x{qty_int} to cart {carrito_id}")
...
def obtener_link_carrito():
    """Finalises the cart and returns a Shopify cart URL, then deletes the temporary cart."""
    limpiar_carritos_viejos()
    ...
    print(f"Cart {carrito_id} finalised with {len(cart_items)} items")
